...
public class ForestMutableWorldState implements MutableWorldState {
  ...
  protected class WorldStateAccount implements Account {
              ...
              (key, value) -> {
                final AccountStorageEntry entry =
                    AccountStorageEntry.create(
                        convertToBytes32(value), key, getStorageTrieKeyPreimage(key).orElse(null));
    ...
    private Optional<Bytes32> getStorageTrieKeyPreimage(final Bytes32 trieKey) {
      return Optional.<Bytes32>ofNullable(newStorageKeyPreimages.get(trieKey))
          ...
          for (final Map.Entry<Bytes32, Bytes32> entry : entries) {
            ...
            if (value.isZero()) {
              ...
              storageTrie.put(
                  ...
                  RLP.encode(out -> out.writeBytes(entry.getValue().trimLeadingZeros())));
            }
          }
